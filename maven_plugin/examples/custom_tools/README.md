...
```yaml
tools:
 - name: com.example.CustomDieTool.rollDie
 - name: com.example.CustomDieTool.CHECK_PRIME_INSTANCE
```
*Note: The agent config can point to either the method name (`rollDie`) or the `FunctionTool` instance name (`CHECK_PRIME_INSTANCE`).*
