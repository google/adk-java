...
public abstract class ApiClient {
  ...
  ApiClient(Optional<String> apiKey, Optional<HttpOptions> customHttpOptions) {
    ...
    // Initialize httpClient after all httpOptions are configured
  ...
  private OkHttpClient createHttpClient(Optional<Integer> timeoutMillis) {
    OkHttpClient.Builder builder = new OkHttpClient.Builder();
    if (timeoutMillis.isPresent()) {
      long timeoutMs = timeoutMillis.get().longValue();
  ...
  /** Returns the OkHttpClient for API calls. */
  OkHttpClient httpClient() {
    return httpClient;
  }
