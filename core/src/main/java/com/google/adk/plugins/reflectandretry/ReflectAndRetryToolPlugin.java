 ...
 * <pre>{@code
 * Runner runner =
 *     new InMemoryRunner(agent, APP_NAME, ImmutableList.of(new ReflectAndRetryToolPlugin(3)));
 * }</pre>
 *
...
public class ReflectAndRetryToolPlugin extends BasePlugin {
   ...
   * {@code maxRetries == 0}, where nothing is counted at all — upstream's {@code
   * _handle_tool_error} returns before its counter runs, and so does this — so the observed count
