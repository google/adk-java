...
final class JsonFormatter {
  ...
  private static ParsedContentObject parseContentObject(Object content, int maxLength) {
    ...
    for (int i = 0; i < parts.size(); i++) {
      ...
      partData.put("part_index", i);
      partData.put("mime_type", "text/plain");
      partData.putNull("uri");
      partData.putNull("text");
      partData.put("part_attributes", "{}");
      partData.put("storage_mode", "INLINE");
      partData.putNull("object_ref");

      // CASE A: It is already a URI (e.g. from user input)
      if (part.fileData().isPresent()) {
        FileData fileData = part.fileData().get();
        partData.put("storage_mode", "EXTERNAL_URI");
        partData.put("uri", fileData.fileUri().orElse(null));
        partData.put("mime_type", fileData.mimeType().orElse(null));
      }
      // CASE B: It is Binary/Inline Data (Image/Blob)
      else if (part.inlineData().isPresent()) {
        // TODO: (b/485571635) Implement GCS offloading here.
        partData.put("text", "[BINARY DATA]");
        partData.put("mime_type", part.inlineData().get().mimeType().orElse(""));
      }
      // CASE C: Text
        ...
        partData.put("mime_type", "application/json");
        partData.put("text", "Function: " + fc.name().orElse("unknown"));
        ObjectNode partAttributes = mapper.createObjectNode();
        partAttributes.put("function_name", fc.name().orElse("unknown"));
        partData.put("part_attributes", partAttributes.toString());
      }
      contentParts.add(partData);
    }
