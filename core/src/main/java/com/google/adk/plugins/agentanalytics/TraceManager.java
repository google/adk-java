...
public final class TraceManager {
  ...
  public String getTraceId(InvocationContext context) {
    ...
    if (ambient.getSpanContext().isValid()) {
      return ambient.getSpanContext().getTraceId();
    }
  ...
  public SpanIds getCurrentSpanAndParent() {
    ...
    Iterator<SpanRecord> descIterator = records.descendingIterator();
    while (descIterator.hasNext()) {
      SpanRecord record = descIterator.next();
      if (!record.spanId().equals(spanId)) {
        parentId = record.spanId();
        break;
      }
    }
  ...
  public @Nullable String getCurrentSpanId() {
    ...
    return records.peekLast().spanId();
  }

  public void recordFirstToken(String spanId) {
    for (SpanRecord record : records) {
      if (record.spanId().equals(spanId)) {
        record.firstTokenTime().compareAndSet(null, Instant.now());
        return;
      }
    }
  }
  ...
  public Optional<Instant> getStartTime(String spanId) {
    for (SpanRecord record : records) {
      if (record.spanId().equals(spanId)) {
        return Optional.of(record.startTime());
      }
    }
    return Optional.empty();
  }
  ...
  public Optional<Instant> getFirstTokenTime(String spanId) {
    for (SpanRecord record : records) {
      if (record.spanId().equals(spanId)) {
        return Optional.ofNullable(record.firstTokenTime().get());
      }
    }
    return Optional.empty();
  }
