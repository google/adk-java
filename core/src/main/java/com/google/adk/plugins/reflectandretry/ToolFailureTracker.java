 ...
 *
 * <p>adk-python nests a {@code dict} per scope inside another {@code dict} and guards both with an
 * {@code asyncio.Lock}, because it mutates them across {@code await} points. Neither is needed
 * here. Nothing ever enumerates the tools within one scope, so the pair is simply the key, and
 * {@link Map#merge} and {@link Map#remove} each apply atomically on a {@link ConcurrentHashMap} —
 * parallel tool failures cannot lose a count.
 */
