...
final class StepResume {
  ...
  static Decision decide(InvocationContext context, Map<String, BaseTool> tools) {
    ...
    // A single event, or a branch the scan cleared: the branch still owes something only when its
    // last event carries calls nothing has answered -- being last is what makes them unanswered.
    Event last = events.get(events.size() - 1);
    if (!last.partial().orElse(false)
        && !last.functionCalls().isEmpty()
        // Only replay a call the CURRENT AGENT authored: a restored session may carry
        // caller-supplied events, and dispatching one would run a tool with no model turn.
  ...
  private static boolean pauseLeftCallsUnanswered(InvocationContext context, List<Event> events) {
    ...
    // Asks whether ANY awaited id is still open, so a partially answered pause keeps waiting.
