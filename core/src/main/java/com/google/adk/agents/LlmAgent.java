...
import static com.google.adk.agents.ConfigAgentUtils.convertToPascalCase;
...
public class LlmAgent extends BaseAgent {
  ...
  public static LlmAgent fromConfig(LlmAgentConfig config, String configAbsPath)
    ...
    try {
      if (config.tools() != null) {
        builder.tools(resolveTools(config.tools(), config.name()));
      }
  ...
  private static BaseTool resolveBuiltInTool(String toolName, ToolConfig toolConfig)
    ...
    try {
      ...
      String className = "com.google.adk.tools." + convertToPascalCase(toolName);
      Class<?> toolClass;
      try {
