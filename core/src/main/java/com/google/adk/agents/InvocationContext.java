...
import com.google.adk.runner.ResumabilityConfig;
...
public class InvocationContext {
  ...
  public boolean shouldPauseInvocation(Event event) {
    ...
    if (event.longRunningToolIds().isEmpty() || event.functionCalls().isEmpty()) {
      return false;
    }
    for (var functionCall : event.functionCalls()) {
      var functionCallId = functionCall.id().orElse(null);
      if (functionCallId != null && event.longRunningToolIds().get().contains(functionCallId)) {
        return true;
      }
    }
    return false;
