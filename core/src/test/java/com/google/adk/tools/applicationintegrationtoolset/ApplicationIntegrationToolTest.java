...
public final class ApplicationIntegrationToolTest {
  ...
  public void declaration_success() {
    ...
    assertThat(declarationOpt.isPresent()).isTrue();
    FunctionDeclaration declaration = declarationOpt.get();
    assertThat(declaration.name()).isEqualTo(Optional.of(FAKE_TOOL_NAME));
    assertThat(declaration.description()).isEqualTo(Optional.of("test summary 1"));

    Optional<Schema> paramsOpt = declaration.parameters();
    assertThat(paramsOpt.isPresent()).isTrue();
    Schema paramsSchema = paramsOpt.get();
    assertThat(paramsSchema.type()).isEqualTo(Optional.of(new Type("OBJECT")));
    assertThat(paramsSchema.properties().isPresent()).isTrue();
    ...
    assertThat(lineItemsSchema.type()).isEqualTo(Optional.of(new Type("ARRAY")));
    assertThat(lineItemsSchema.items().isPresent()).isTrue();

    Schema itemSchema = lineItemsSchema.items().get();
    assertThat(itemSchema.type()).isEqualTo(Optional.of(new Type("OBJECT")));
    assertThat(itemSchema.properties().isPresent()).isTrue();
  ...
  public void declaration_operationNotFound_returnsEmpty() {
    ...
    assertThat(declarationOpt.isPresent()).isFalse();
  }
