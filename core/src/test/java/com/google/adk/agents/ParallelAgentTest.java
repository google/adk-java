...
public final class ParallelAgentTest {
  ...
  public void runAsync_resumable_subAgentEscalates_emitsEndOfAgent() {
    // Authored by the sub-agent itself: only a direct sub-agent's escalation addresses this agent.
    BaseAgent escalating =
        createSubAgent(
            "escalating", createEscalateEvent("esc").toBuilder().author("escalating").build());
    ParallelAgent parallelAgent =
