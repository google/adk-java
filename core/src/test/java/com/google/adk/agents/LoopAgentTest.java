...
public final class LoopAgentTest {
  ...
  // ---- Resumability: durable checkpoint resume. ----
  ...
  // Restarting the count would loop past the cap, so an unreadable value fails the resume instead.
  @Test
  public void runAsync_resumingWithNonNumericTimesLooped_throws() {
    TestBaseAgent subAgent =
        createSubAgent("sub", createEvent("e").toBuilder().author("sub").build());
    LoopAgent loopAgent =
        LoopAgent.builder().name("loop").subAgents(subAgent).maxIterations(2).build();
    InvocationContext context = createResumableInvocationContext(loopAgent);
    context.setAgentState(
        "loop",
        ImmutableMap.of("current_sub_agent", "sub", "times_looped", "two"),
        /* endOfAgent= */ false);

    var resumed = loopAgent.runAsync(context).toList();

    assertThrows(IllegalStateException.class, resumed::blockingGet);
  }

  @Test
  public void runAsync_resumingAtIterationCap_endsWithoutRunningSubAgent() {
    TestBaseAgent subAgent =
        createSubAgent("sub", createEvent("e").toBuilder().author("sub").build());
    LoopAgent loopAgent =
        LoopAgent.builder().name("loop").subAgents(subAgent).maxIterations(2).build();
    InvocationContext context = createResumableInvocationContext(loopAgent);
    // Resume with the loop already at the iteration cap (2 of 2 done).
    context.setAgentState(
        "loop",
        ImmutableMap.of("current_sub_agent", "sub", "times_looped", 2),
        /* endOfAgent= */ false);

    List<Event> events = loopAgent.runAsync(context).toList().blockingGet();

    // Ends immediately: end-of-agent is emitted and no sub-agent runs.
    assertThat(
            events.stream()
                .anyMatch(event -> event.author().equals("loop") && event.actions().endOfAgent()))
        .isTrue();
    assertThat(events.stream().anyMatch(event -> event.author().equals("sub"))).isFalse();
  }

  @Test
  public void runAsync_resumable_belowIterationCap_loopsExactlyMaxIterations() {
    // Guards the cap comparison timesLooped >= maxIterations from below the cap.
    TestBaseAgent subAgent =
        createSubAgent(
            "sub", () -> Flowable.just(createEvent("e").toBuilder().author("sub").build()));
    LoopAgent loopAgent =
        LoopAgent.builder().name("loop").subAgents(subAgent).maxIterations(2).build();
    InvocationContext context = createResumableInvocationContext(loopAgent);
    ...
    assertThat(events.stream().filter(event -> event.author().equals("sub")).count()).isEqualTo(2L);
    assertThat(
            events.stream()
                .anyMatch(event -> event.author().equals("loop") && event.actions().endOfAgent()))
        .isTrue();
