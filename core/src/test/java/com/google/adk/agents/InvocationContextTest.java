...
public final class InvocationContextTest {
  ...
  // The two flags now select different flows: the shim runs the legacy one, so it must not report
  // resumable. Replaces the equivalence the flags had when the shim first started selecting
  // resumption.
  @Test
  ...
  @Test
  public void hasUnansweredPausedCall_partiallyAnsweredParallelCalls_returnsTrue() {
    ...
    assertThat(context.hasUnansweredPausedCall()).isTrue();
  }

  @Test
  public void hasUnansweredPausedCall_allParallelCallsAnswered_returnsFalse() {
    Session eventSession = Session.builder("s").build();
    eventSession.events().add(twoLongRunningCallsEvent());
    eventSession.events().add(functionResponseEvent("a", "approve_a"));
    ...
    assertThat(context.hasUnansweredPausedCall()).isFalse();
  }

  @Test
  public void hasUnansweredPausedCall_noPausedCall_returnsFalse() {
    Session eventSession = Session.builder("s").build();
    eventSession
        .events()
