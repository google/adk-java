...
public final class InvocationContextTest {
  ...
  @Test
  public void hasUnansweredPausedCall_singleCallAnswered_returnsFalse() {
    Session eventSession = Session.builder("s").build();
    eventSession
        .events()
        .add(
            Event.builder()
                ...
                .content(
                    Content.fromParts(
                        Part.builder()
                            .functionCall(FunctionCall.builder().id("c1").name("task").build())
                            .build()))
                .longRunningToolIds(ImmutableSet.of("c1"))
                .build());
    eventSession.events().add(functionResponseEvent("c1", "task"));
    InvocationContext context = resumableContext(eventSession, "inv");
