...
public final class InvocationContextTest {
  ...
  @Test
  public void confirmationApprover_setOnTheApp_reachesTheInvocationContext() {
    // Without this, deleting the Runner wiring leaves every other test green while an operator's
    // policy silently degrades to the default.
    App app =
        App.builder()
            .name("test_app")
            .rootAgent(mockAgent)
    ...
    assertThat(app.confirmationApprover()).isEqualTo(ConfirmationApprover.AUTHENTICATED_ONLY);
    assertThat(App.builder().name("a").rootAgent(mockAgent).build().confirmationApprover())
        .isEqualTo(ConfirmationApprover.REJECT_UNAUTHENTICATED);
  }

  @Test
  public void confirmationApprover_defaultOnTheBuilder_rejectsUnauthenticated() {
    assertThat(contextOnBranch(null).confirmationApprover())
        .isEqualTo(ConfirmationApprover.REJECT_UNAUTHENTICATED);
  }
