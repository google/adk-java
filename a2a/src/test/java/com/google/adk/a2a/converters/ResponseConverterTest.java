...
public final class ResponseConverterTest {
  ...
  @Test
  public void isCompleted_withTaskEventCompleted_returnsTrue() {
    ...
    assertThat(ResponseConverter.isCompleted(event)).isTrue();
  }

  @Test
  public void isCompleted_withTaskEventWorking_returnsFalse() {
    TaskStatus status = new TaskStatus(TaskState.WORKING);
    Task task = testTask().status(status).build();
    TaskEvent event = new TaskEvent(task);
    assertThat(ResponseConverter.isCompleted(event)).isFalse();
  }
  ...
  public void clientEventToEvent_withMessageEvent_returnsEvent() {
    ...
    Event event = ResponseConverter.clientEventToEvent(messageEvent, invocationContext);
  ...
  @Test
  public void taskToEvent_withNullTask_returnsEmptyEvent() {
    Event event = ResponseConverter.taskToEvent(null, invocationContext);
    assertThat(event).isNotNull();
    assertThat(event.content().get().parts().get()).isEmpty();
  }

  @Test
  public void finalTaskToEvent_convertsEvent() {
    Message eventMessage =
        new Message.Builder()
            .role(Message.Role.AGENT)
            .parts(ImmutableList.of(new TextPart("Final message")))
            .build();
    TaskStatus status = new TaskStatus(TaskState.COMPLETED, eventMessage, null);
    TaskStatusUpdateEvent updateEvent =
        new TaskStatusUpdateEvent("task-id-1", status, "context-1", false, null);

    Event event = ResponseConverter.finalTaskToEvent(updateEvent, invocationContext);
    assertThat(event).isNotNull();
    assertThat(event.turnComplete()).hasValue(true);
    assertThat(event.content().get().parts().get().get(0).text()).hasValue("Final message");
  }
  ...
  public void clientEventToEvent_withTaskUpdateEventAndThought_returnsThoughtEvent() {
    ...
    Event resultEvent = ResponseConverter.clientEventToEvent(event, invocationContext);
