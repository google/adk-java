...
public class StaleAgent {
  ...
  private String loadAndFormatPrompt() {
    try (InputStream is = getClass().getResourceAsStream("/PROMPT_INSTRUCTION.txt")) {
      ...
      return template
          ...
          .replace("{WAITING_ON_REPORTER_LABEL}", StaleBotSettings.WAITING_ON_REPORTER_LABEL)
          .replace(
  ...
  @Schema(
      ...
      description = "Retrieves the comprehensive state of a GitHub issue using GraphQL.")
  public Map<String, Object> getIssueState(
      @Schema(name = "item_number", description = "The GitHub issue number") int itemNumber) {
    try {
  ...
  @Schema(name = "add_label_to_issue", description = "Adds a label to the issue.")
  public Map<String, Object> addLabelToIssue(
      @Schema(name = "item_number", description = "The GitHub issue number") int itemNumber,
      @Schema(name = "label_name", description = "The name of the label") String labelName) {
  ...
  @Schema(name = "remove_label_from_issue", description = "Removes a label from the issue.")
  public Map<String, Object> removeLabelFromIssue(
      @Schema(name = "item_number", description = "The GitHub issue number") int itemNumber,
      @Schema(name = "label_name", description = "The name of the label") String labelName) {

    logger.debug("Removing label '{}' from issue #{}", labelName, itemNumber);
    String encodedLabel = URLEncoder.encode(labelName, StandardCharsets.UTF_8).replace("+", "%20");
    String url =
        String.format(
    ...
    String comment =
        String.format(
            "This issue has been automatically marked as stale because there is no recent activity for %s days "
                + "after a maintainer response. It will be closed if no further activity occurs within %s days.",
            staleDays, closeDays);
  ...
  @Schema(name = "close_as_stale", description = "Closes the issue as not planned/stale.")
