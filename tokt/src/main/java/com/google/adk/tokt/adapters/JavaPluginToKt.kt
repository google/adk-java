 ...
 * ([KtInvocationContextToJavaView] / [ktCallbackContextToJava] / [ktToolContextToJava]), invokes
 * Java plugin, and awaits its RxJava result as a coroutine.
 *
 * The Java agent each callback sees is the running engine agent wrapped via [javaAgentView], so no
 * root agent has to be supplied and the same instance is reused for the whole invocation.
 * Tool-level callbacks apply only to Kt-backed Java tools ([JavaToolToKt]); native Kotlin tools
...
internal class JavaPluginToKt(private val plugin: JavaPlugin) : KtPlugin {
  ...
  /**
   * Maps a Java removal sentinel, written through the live callback state, to the Kotlin sentinel
   * so the engine recognizes the deletion.
   */
  ...
  override suspend fun beforeAgent(
    ...
    val override = onIo {
      plugin.beforeAgentCallback(javaAgent, ktCallbackContextToJava(context, javaAgent))
    }
  ...
  override suspend fun afterAgent(context: KtCallbackContext): CallbackChoice<Unit, KtContent> {
    val javaAgent = javaAgentView(context)
    val override = onIo {
      plugin.afterAgentCallback(javaAgent, ktCallbackContextToJava(context, javaAgent))
    }
    reconcileState(context)
    return if (override != null) CallbackChoice.Break(ContentCodec.fromJava(override))
    else CallbackChoice.Continue(Unit)
  ...
  override suspend fun beforeModel(
    ...
    val override = onIo {
      plugin.beforeModelCallback(ktCallbackContextToJava(context, javaAgentView(context)), builder)
    }
    reconcileState(context)
  ...
  override suspend fun afterModel(
    ...
    val override = onIo {
      plugin.afterModelCallback(
        ktCallbackContextToJava(context, javaAgentView(context)),
  ...
  override suspend fun beforeTool(
    ...
    val override = onIo {
      plugin.beforeToolCallback(javaTool, mutableArgs, ktToolContextToJava(context))
    }
  ...
  override suspend fun afterTool(
    ...
    val override = onIo {
      plugin.afterToolCallback(javaTool, args, ktToolContextToJava(context), result)
    }
    reconcileToolState(context)
  ...
  override suspend fun onToolError(
    ...
    val fallback = onIo {
      plugin.onToolErrorCallback(javaTool, args, ktToolContextToJava(context), error)
    }
    reconcileToolState(context)
