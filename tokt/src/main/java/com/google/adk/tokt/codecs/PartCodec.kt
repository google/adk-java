 ...
 * call/response parts (each preserving its name, id, and arguments; the inline byte array is shared
 * by reference), the model "thought" marker/signature, video metadata, and part metadata. Shared by
 * [ContentCodec] and the artifact service.
 *
 * Returns null for an empty part and for one carrying only a field the Kotlin [KtPart] has no
 * counterpart for - `executableCode`, `codeExecutionResult`, `toolCall`, `toolResponse` - which
  ...
  private fun warnUnmappedKind(part: GenaiPart) {
    val kind =
      when {
        ...
        part.toolCall().isPresent -> "toolCall"
        part.toolResponse().isPresent -> "toolResponse"
