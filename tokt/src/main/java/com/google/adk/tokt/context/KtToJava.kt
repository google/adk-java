 ...
 */
@OptIn(FrameworkInternalApi::class)
internal fun ktInvocationContextJavaBuilder(
  ic: KtInvocationContext,
  agent: JavaBaseAgent? = null,
  ...
  val builder =
    JavaInvocationContext.builder()
      ...
      // Wire the agent so a Java tool reading ToolContext.agentName() / invocationContext.agent()
      // does not hit a null. Callers adapting a whole Java agent pass the real agent; everyone else
      // gets the memoized inspection-only view of the running Kotlin agent.
      .agent(agent ?: javaAgentView(ic))
  ...
  // Set the invocation-scoped scratch map on the builder as well as overriding the accessor: the
  // Java flow copies the context for sub-agents, and a copy carries the field, not the override.
  // PersistBarrier lives in here, so losing it across a copy breaks the step handshake.
  builder.callbackContextData(ic.frameworkData.callbackContextData)
  return builder
}
...
internal class KtInvocationContextToJavaView(
  ...
  constructor(ic: KtInvocationContext) : this(ktInvocationContextJavaBuilder(ic), ic)
