...
internal object EventCodec {
   ...
   *
   * `EventActions.agentState` and `rewindBeforeInvocationId` are dropped: ADK Java has no field for
   * either. Behind a persistent Java session service that costs resumability, since the engine
   * reads its resumption state back from the stored event.
   */
