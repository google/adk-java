 ...
 * driving the agent.
 *
 * A few things to know before relying on them:
 ...
 * - A bridged plugin's `onRunErrorCallback` does not fire: the engine surfaces run-level errors
 *   through the returned event stream to the caller, not to plugins. The `onModelErrorCallback` and
 *   `onToolErrorCallback` recovery hooks do fire.
 * - A bridged tool's or plugin's `requestedAuthConfigs` and `deletedArtifactIds` writes do not
 *   reach the engine, whose event actions have no equivalent; a tool's `requestedToolConfirmations`
 *   does cross.
 */
object JavaAdkToKt {
