...
internal class KtAgentToJava(internal val ktAgent: KtBaseAgent) :
  JavaBaseAgent(
    ktAgent.name,
    ktAgent.description,
    // Wrap each child rather than unwrapping via ktAgentAsJava: the Java BaseAgent constructor sets
    // each sub-agent's parentAgent, so unwrapping would repoint a real Java agent's parent at this
    // throwaway view. (Unwrapping is still right for the agent handed to a plugin - only read.)
    // Views all the way down the tree, then, and unwrapped only at the top: a plugin walking
    // subAgents() gets a view, not the same instance it receives as its `agent` argument.
