...
class KtRunnerInteropTest {
   ...
   */
  private class AppendNoteJavaTool :
    JavaBaseTool("append_note", "appends a note through the live session") {
    val observed = ConcurrentHashMap<String, Any>()

    override fun declaration(): Optional<GenaiFunctionDeclaration> =
      Optional.of(GenaiFunctionDeclaration.builder().name("append_note").build())
  ...
  private fun appendNoteAgent(tool: AppendNoteJavaTool) =
    KtLlmAgent(
      ...
      model =
        JavaAdkToKt.asKtModel(
          SequentialJavaModel(
            listOf(modelFunctionCall("append_note", emptyMap()), modelText("done"))
          )
        ),
  ...
  fun liveEventsView_add_appendsToKotlinSession() {
    ...
    assertTrue(KtBackedEventsView(session).add(event), "add must report the change")
  ...
  fun liveEventsView_otherMutations_throwUnsupportedOperation() {
    ...
    val events = KtBackedEventsView(session)
  ...
  @Test
  fun asJavaRunner_appendEventOnASnapshot_updatesItInPlaceLikeAJavaService() = runBlocking {
    // Snapshot appends follow ADK Java's in-place contract and keep a GetSessionConfig window.
    val ktRunner =
      KtInMemoryRunner(
        KtLlmAgent(
          name = "a",
    ...
    val event =
      JavaEvent.builder()
        ...
        .actions(
          JavaEventActions.builder()
            .stateDelta(mutableMapOf<String, Any>("color" to "teal"))
            .build()
        )
        .build()
