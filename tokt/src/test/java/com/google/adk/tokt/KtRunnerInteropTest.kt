...
class KtRunnerInteropTest {
  ...
  fun ktRunner_pluginOverAdaptedJavaAgent_receivesTheOriginalInstance() = runBlocking {
    // The opaque adapter pair unwraps rather than stacking, so a Java plugin gets back the same
    // agent it was configured with (Java agents compare by identity, which a plugin pairing its
    // before/after callbacks relies on). A globalInstruction keeps the agent on the opaque path,
    // since a plain LlmAgent now translates to a native engine agent.
    val javaAgent =
      JavaLlmAgent.builder()
        .name("java_agent")
        .description("adapted java agent")
        .globalInstruction("run opaquely")
  ...
  fun ktRunner_adaptedJavaAgent_carriesMaxLlmCallsIntoTheJavaBudget() = runBlocking {
    withTimeout(ADAPTED_AGENT_TIMEOUT) {
      ...
      // model that never stops calling its tool must be cut off after exactly that many calls. A
      // globalInstruction keeps the agent on this opaque path (a plain LlmAgent now runs natively,
