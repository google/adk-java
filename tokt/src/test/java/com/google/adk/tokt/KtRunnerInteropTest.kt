...
class KtRunnerInteropTest {
  ...
  /** A Java toolset that records when it is closed, to prove close() reaches Java toolsets. */
  private class CloseRecordingJavaToolset : JavaBaseToolset {
    var closed = false

    override fun getTools(readonlyContext: JavaReadonlyContext?): Flowable<JavaBaseTool> =
      Flowable.just<JavaBaseTool>(JavaToolsetTool())
  ...
  /** A Java tool that writes a session-state value via its tool context. */
  private class JavaSetStateTool : JavaBaseTool("set_state", "sets a state value") {
    override fun declaration(): Optional<GenaiFunctionDeclaration> =
      Optional.of(GenaiFunctionDeclaration.builder().name("set_state").build())
  ...
  /** A Java tool that reads a session-state value via its tool context. */
  private class JavaGetStateTool : JavaBaseTool("get_state", "reads a state value") {
    override fun declaration(): Optional<GenaiFunctionDeclaration> =
      Optional.of(GenaiFunctionDeclaration.builder().name("get_state").build())
    ...
    override fun runAsync(
      ...
      Single.just(mapOf("color" to (toolContext.state()["color"] ?: "MISSING")))
  }

  /**
   * A Java tool marked long-running: its result is a placeholder awaiting an out-of-band answer.
   */
  private class JavaLongRunningTool :
    JavaBaseTool("await_approval", "awaits an out-of-band approval") {
    override fun declaration(): Optional<GenaiFunctionDeclaration> =
      Optional.of(GenaiFunctionDeclaration.builder().name("await_approval").build())

    override fun longRunning(): Boolean = true
  ...
  @Test
  fun ktRunner_pluginOverAdaptedJavaAgent_receivesTheOriginalInstance() = runBlocking {
    // The adapter pair unwraps rather than stacking, so a Java plugin bridged onto the engine is
    // handed back the very agent it was configured with. Java agents compare by identity, so a
    // plugin pairing before/after callbacks by agent depends on this.
    val javaAgent =
      JavaLlmAgent.builder()
        .name("java_agent")
        .description("adapted java agent")
        .model(SequentialJavaModel(listOf(modelText("done"))))
        .build()
    val seen = AtomicReference<JavaBaseAgent>()
    val plugin =
      object : JavaBasePlugin("agent-identity") {
        override fun beforeAgentCallback(
          agent: JavaBaseAgent,
          callbackContext: JavaCallbackContext,
        ): Maybe<GenaiContent> {
          seen.compareAndSet(null, agent)
          return Maybe.empty()
        }
      }
    val runner =
    ...
    assertTrue(
      seen.get() === javaAgent,
      "the plugin should receive the original Java agent, not a view wrapping it",
    )
  }
  ...
  fun javaAdkToKt_convertsEntireCollections() {
    ...
    // Agents: order and identity preserved, each adapted onto the engine.
    val ktAgents =
      JavaAdkToKt.asKtAgents(
        listOf(
          JavaLlmAgent.builder().name("first").description("d").build(),
          JavaLlmAgent.builder().name("second").description("d").build(),
        )
      )
    assertEquals(listOf("first", "second"), ktAgents.map { it.name })

    // Empty collections convert to empty.
    assertTrue(JavaAdkToKt.asKtTools(emptyList()).isEmpty())
    assertTrue(JavaAdkToKt.asKtAgents(emptyList()).isEmpty())
  }

  @Test
  fun asKtAgent_javaAgentWithoutDescription_mapsToEmptyDescription() {
    // A Java agent's description may be null; adapting it must not throw and defaults to "".
    val javaAgent =
      JavaLlmAgent.builder()
        .name("nodesc")
        .model(SequentialJavaModel(listOf(modelText("x"))))
        .build()
    assertEquals("", JavaAdkToKt.asKtAgent(javaAgent).description)
  }
  ...
  @Test
  fun asJavaRunner_close_closesTheAdaptedJavaAgentsToolsets() {
    // A Kotlin runner over an *adapted* Java agent: the agent is an opaque leaf on the Kotlin side,
    // so ktRunner.close() cannot see its Java toolsets - only super.close() reaches them.
    val toolset = CloseRecordingJavaToolset()
    val javaAgent =
      JavaLlmAgent.builder()
        .name("a")
    ...
    val javaRunner =
      KotlinAdkToJava.asJavaRunner(
        KtInMemoryRunner(JavaAdkToKt.asKtAgent(javaAgent), appName = "app")
      )

    javaRunner.close().blockingAwait()

    assertTrue(toolset.closed, "closing the wrapper should close the adapted Java agent's toolsets")
  }

  @Test
  fun asJavaRunner_runLiveThrows_andAgentReturnsAnInspectionView() {
    val ktRunner =
      KtInMemoryRunner(
        agent =
    ...
    // Live mode is not bridged; it must fail loudly, not silently do the wrong thing.
  ...
  @Test
  fun ktRunner_runsAdaptedJavaAgent_multiStepSeesToolResult() = runBlocking {
    // Bounded: this code's characteristic failure is a barrier that never releases, and a
    // bare runBlocking would surface that as an opaque target timeout.
    withTimeout(ADAPTED_AGENT_TIMEOUT) {
      // A whole Java LlmAgent (model -> tool -> model) runs on the Kotlin engine via asKtAgent. The
      // adapter drives the Java PersistBarrier on the runner's behalf, keeping the flow's session
      // current across steps, so the second model call sees the tool's function response.
      val requestFunctionResponseCounts = CopyOnWriteArrayList<Int>()
      val model =
        object : JavaBaseLlm("java-model") {
          ...
          override fun generateContent(
            ...
            requestFunctionResponseCounts.add(
              llmRequest.contents().count { c ->
                c.parts().getOrNull().orEmpty().any { it.functionResponse().isPresent }
              }
            )
            val content =
              if (step.getAndIncrement() == 0) modelFunctionCall("java_echo", mapOf("text" to "hi"))
              else modelText("java agent done")
            return Flowable.just(JavaLlmResponse.builder().content(content).build())
          }
      ...
      val javaAgent =
        JavaLlmAgent.builder()
          ...
          .tools(JavaEchoTool())
          .build()
      val runner = KtInMemoryRunner(JavaAdkToKt.asKtAgent(javaAgent), appName = "app")
      ...
      assertTrue(
        ...
        "the adapted Java agent's tool should have run",
      )
      assertTrue(
        events.any { e -> e.content?.parts?.any { it.text == "java agent done" } == true },
        "the adapted Java agent should produce its final answer",
      )
      // The second model step's request must include the tool's function response, proving the Java
      // flow's session was kept current across steps.
      assertEquals(2, requestFunctionResponseCounts.size, "expected two model steps")
      assertEquals(
        0,
        requestFunctionResponseCounts[0],
        "step 1 request carries no function response",
      )
      assertTrue(
        requestFunctionResponseCounts[1] >= 1,
        "step 2 request must include the tool's function response (session kept current)",
      )
    }
  }

  @Test
  fun ktRunner_adaptedJavaAgent_seesStateSetEarlierInSameTurn() = runBlocking {
    // Bounded: this code's characteristic failure is a barrier that never releases, and a
    // bare runBlocking would surface that as an opaque target timeout.
    withTimeout(ADAPTED_AGENT_TIMEOUT) {
      // A tool sets session state in step 1; a tool in step 2 reads it back. That works only if the
      // adapted agent's Java session is a LIVE state view over the Kotlin one, not a turn-start
      // snapshot.
      val javaAgent =
        JavaLlmAgent.builder()
          .name("stateful_agent")
          .description("adapted stateful java agent")
          .model(
            SequentialJavaModel(
              listOf(
                modelFunctionCall("set_state", emptyMap()),
                modelFunctionCall("get_state", emptyMap()),
                modelText("done"),
              )
            )
          )
          .tools(JavaSetStateTool(), JavaGetStateTool())
          .build()
      val runner = KtInMemoryRunner(JavaAdkToKt.asKtAgent(javaAgent), appName = "app")
      ...
      val getStateResponse =
        events
          .flatMap { it.content?.parts.orEmpty() }
          .mapNotNull { it.functionResponse }
          .firstOrNull { it.name == "get_state" }
      assertEquals(
        "teal",
        getStateResponse?.response?.get("color"),
        "step 2's tool must see the state a step 1 tool set (live state view)",
      )
    }
  }

  @Test
  fun ktRunner_adaptedJavaAgent_carriesMaxLlmCallsIntoTheJavaBudget() = runBlocking {
    withTimeout(ADAPTED_AGENT_TIMEOUT) {
      // The Kotlin RunConfig's maxLlmCalls has to reach the Java context, or the adapted agent runs
      // unbounded. It lands on the Java flow's own counter, so the cap is enforced Java-side: a
      // model that never stops calling its tool must be cut off after exactly that many calls.
      ...
      val model =
        object : JavaBaseLlm("java-model") {
          override fun generateContent(
            ...
            return Flowable.just(
              JavaLlmResponse.builder()
                .content(modelFunctionCall("java_echo", mapOf("text" to "again")))
                .build()
            )
          }
      ...
      val javaAgent =
        JavaLlmAgent.builder()
          .name("java_agent")
          .description("loops until the budget stops it")
          .model(model)
          .tools(JavaEchoTool())
          .build()
      val runner = KtInMemoryRunner(JavaAdkToKt.asKtAgent(javaAgent), appName = "app")
      ...
      assertFailsWith<LlmCallsLimitExceededException> {
        runner
          .runAsync(
            ...
            sessionId = "s",
            newMessage = KtContent.fromText("user", "go"),
            runConfig = KtRunConfig(maxLlmCalls = 2),
          )
          .toList()
      }
      assertEquals(2, modelCalls.get(), "the Java side must stop at the carried maxLlmCalls")
    }
  }

  @Test
  fun ktRunner_adaptedJavaAgent_pausesOnPendingLongRunningCall() = runBlocking {
    // Bounded: this code's characteristic failure is a barrier that never releases, and a
    // bare runBlocking would surface that as an opaque target timeout.
    withTimeout(ADAPTED_AGENT_TIMEOUT) {
      // The Java flow pauses on a pending long-running call only when its context is resumable; if
      // the adapter drops that, it loops back to the model instead of awaiting the answer.
      val modelCalls = AtomicInteger()
      val model =
        object : JavaBaseLlm("java-model") {
          override fun generateContent(
            ...
            val content =
              if (modelCalls.incrementAndGet() == 1) modelFunctionCall("await_approval", emptyMap())
              else modelText("should not be reached")
            return Flowable.just(JavaLlmResponse.builder().content(content).build())
          }
      ...
      val javaAgent =
        JavaLlmAgent.builder()
          .name("approver")
          .description("waits for an out-of-band approval")
          .model(model)
          .tools(JavaLongRunningTool())
          .build()
      val runner =
        KtInMemoryRunner(
          KtApp(
      ...
      assertEquals(
        1,
        modelCalls.get(),
        "the flow must pause on the pending long-running call, not loop",
      )
      assertTrue(
        events.any { e ->
          e.content?.parts?.any { it.functionCall?.name == "await_approval" } == true
        },
        "the long-running call should be surfaced to the caller",
      )
    }
  }

  private companion object {
    /** Generous, but finite: enough that a slow machine passes and a stuck barrier still fails. */
    const val ADAPTED_AGENT_TIMEOUT = 60_000L
